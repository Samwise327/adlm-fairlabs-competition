...
#######################
      #tested
#######################
#selected_races_before <- subset(race_pct, pct > 1 & BA_intervention == "Before")$maternal_race
selected_races_before <- subset(race_pct, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("tested ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_tested_before <- glm(formula_str, data=fairlabs_data_before, na.action = "na.exclude", family = binomial)
summary(m.glm_tested_before)

#selected_races_after <- subset(race_pct, pct > 1 & BA_intervention == "After")$maternal_race
selected_races_after <- subset(race_pct, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("tested ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_tested_after <- glm(formula_str, data=fairlabs_data_after, na.action = "na.exclude", family = binomial)
...
#######################
        #pos
#######################
#selected_races_before <- subset(race_pct_tested, pct > 1 & BA_intervention == "Before")$maternal_race
selected_races_before <- subset(race_pct_tested, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("tested_pos ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_tested_pos_before <- glm(formula_str, data=fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
summary(m.glm_tested_pos_before)


selected_races_after <- subset(race_pct_tested, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("tested_pos ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_tested_pos_after <- glm(formula_str, data=fairlabs_data_tested_after, na.action = "na.exclude", family = binomial)
...
#######################
selected_races_before <- subset(race_pct_tested, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("tested_pos_wo_mj ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_tested_pos_wo_mj_before <- glm(formula_str, data=fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
summary(m.glm_tested_pos_wo_mj_before)

selected_races_after <- subset(race_pct_tested, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("tested_pos_wo_mj ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_tested_pos_wo_mj_after <- glm(formula_str, data=fairlabs_data_tested_after, na.action = "na.exclude", family = binomial)
...
#######################
selected_races_before <- subset(race_pct_tested, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("reported ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_reported_before <- glm(formula_str, data=fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
summary(m.glm_reported_before)

selected_races_after <- subset(race_pct_tested, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("reported ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_reported_after <- glm(formula_str, data=fairlabs_data_tested_after, na.action = "na.exclude", family = binomial)
...
#######################
selected_races_before <- subset(race_pct_tested, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("cps_w_pos ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_cps_w_pos_before <- glm(formula_str, data=fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
summary(m.glm_cps_w_pos_before)

selected_races_after <- subset(race_pct_tested, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("cps_w_pos ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_cps_w_pos_after <- glm(formula_str, data=fairlabs_data_tested_after, na.action = "na.exclude", family = binomial)
...
#######################
selected_races_before <- subset(race_pct_tested, cnt > 10 & BA_intervention == "Before")$maternal_race
formula_str <- paste("cps_w_pos_wo_mj ~ maternal_age +", paste(selected_races_before, collapse = " + "))
m.glm_cps_w_pos_wo_mj_before <- glm(formula_str, data=fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
summary(m.glm_cps_w_pos_wo_mj_before)

selected_races_after <- subset(race_pct_tested, cnt > 10 & BA_intervention == "After")$maternal_race
formula_str <- paste("cps_w_pos_wo_mj ~ maternal_age +", paste(selected_races_after, collapse = " + "))
m.glm_cps_w_pos_wo_mj_after <- glm(formula_str, data=fairlabs_data_tested_after, na.action = "na.exclude", family = binomial)
summary(m.glmcps_w_pos_wo_mj_after)

```

## ML BEFORE / AFTER INTERVENTION - (original) not working depending on order?
...
m.glm_tested_before <- glm(tested ~ maternal_age + white_yn + black_yn + asian_yn + pi_yn + native_yn + ni_yn + other_yn, data=fairlabs_data_before, na.action = "na.exclude", family = binomial, control = list(maxit = 200))
summary(m.glm_tested_before)

m.glm_tested_after <- glm(tested ~ maternal_age + black_yn + white_yn + asian_yn + pi_yn + native_yn + other_yn + ni_yn, data=fairlabs_data_after, na.action = "na.exclude", family = binomial)
summary(m.glm_tested_after)
...
m.glm_pos_wo_mj_before <- glm(tested_pos_wo_mj ~ maternal_age + black_yn + white_yn + asian_yn + pi_yn + ni_yn + native_yn, data= fairlabs_data_tested_before, na.action = "na.exclude", family = binomial)
...
x_vars <- c("m.glm_tested_before", "m.glm_tested_pos_before", 
            "m.glm_tested_pos_wo_mj_before", "m.glm_reported_before", 
            "m.glm_cps_w_pos_before", "m.glm_cps_w_pos_wo_mj_before")
...
x_vars2 <- c("m.glm_tested_after", "m.glm_tested_pos_after", 
             "m.glm_tested_pos_wo_mj_after", "m.glm_reported_after", 
             "m.glm_cps_w_pos_after", "m.glm_cps_w_pos_wo_mj_after")
